...
#define PLANET_RADIUS 10000. /* radius of the planet */
#define ATMOS_RADIUS 11000. /* radius of the atmosphere */
// scattering coeffs
...
*/
  ...
  void main() {
    d = readDepth(tDepth, vUv);
    diffuse = texture2D(tDiffuse, vUv);
    dif = vec4(diffuse.rgb * (1.0 - d), 1.0);
    vec3 posWS = _ScreenToWorld(vec3(vUv, d));
    vec3 cameraDirection = normalize(posWS - cameraPosition);
  
    // get the camera vector
    vec3 camera_vector = cameraDirection;
...
export class Atmosphere{
    ...
    createcomposer(){
        ...
        const size     = renderer.renderer.getDrawingBufferSize( new THREE.Vector2() );
        this.target    = new THREE.WebGLRenderTarget( size.width, size.height, { samples: 1 } );
        let renderPass = new RenderPass(renderer.scene_, renderer.camera_);

        const colorCorrectionPass = new ShaderPass( ColorCorrectionShader );
        const outputPass          = new OutputPass();
        const composer            = new EffectComposer(renderer.renderer,this.target);
       ...
       renderer.renderer.setRenderTarget(this.target);
        ...
        this.depthPass  = new THREE.ShaderMaterial({
          vertexShader: postVertexShader,
          fragmentShader: postFragmentShader,
          uniforms: {
            cameraNear: {
              value: renderer.camera_.near
            },
            cameraFar: {
              value: renderer.camera_.far
            },
            cameraPosition: {
              value: renderer.camera_.position
            },
            cameraForward: {
              value: null
            },
        ...
        this. postQuad  = new THREE.Mesh(postPlane, this.depthPass);
      ...
      run() {
        ...
        renderer.renderer.render(renderer.scene_, renderer.camera_);
        renderer.renderer.setRenderTarget(null);
        const forward = new THREE.Vector3();
        renderer.camera_.getWorldDirection(forward);
        this.depthPass.uniforms.inverseProjection.value = renderer.camera_.projectionMatrixInverse;
        this.depthPass.uniforms.inverseView.value       = renderer.camera_.matrixWorld;
        this.depthPass.uniforms.tDiffuse.value          = this.target.texture;
        this.depthPass.uniforms.tDepth.value            = this.target.depthTexture;
        this.depthPass.uniforms.cameraNear.value        = renderer.camera_.near;
        this.depthPass.uniforms.cameraFar.value         = renderer.camera_.far;
        this.depthPass.uniforms.cameraForward.value     = forward;
        this.depthPass.uniformsNeedUpdate               = true;
        renderer.renderer.render(this.postScene, this.postCamera);
      };
