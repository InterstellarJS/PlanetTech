...
import {snoise3D,fbmNoise}  from  './../shaders/glslFunctions'
...
export class CubeMap{
    ...
    noiseFBM(op,params){
        this.cube.children.map((p)=>{
            ...
            var newPostion = NODE.float(100.0).mul((NODE.positionWorld.sub(cnt_).normalize())).add(cnt_) 
            var wp = (NODE.modelViewMatrix.mul(NODE.vec4(newPostion,1.0))).xyz;
            if (!params.hasOwnProperty('v_')) {
              params.v_ = wp.mul(.1).mul(.8);
            }
            if(p.material.colorNode){
              p.material.colorNode = p.material.colorNode[op](fbmNoise.call(params).add(0))
            }else{
              p.material.colorNode = fbmNoise.call(params).add(0)
            }
        })
    }
